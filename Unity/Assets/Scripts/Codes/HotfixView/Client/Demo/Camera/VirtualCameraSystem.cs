    ...
    public static class VirtualCameraSystem
    {
        ...
        public class VirtualCameraGizmosUpdateSystem : GizmosUpdateSystem<VirtualCamera>
        {
            protected override void GizmosUpdate(VirtualCamera self)
            {
                ...
                //Camera Target
                foreach (var kv in self.targetIds)
                {
                    CameraTarget _target = self.GetChild<CameraTarget>(kv.Value);
                    Unit unit = Root.Instance.Get(_target.UnitId) as Unit;
                    if (unit == null)
                    {
                        return;
                    }
                    GameObject go = unit.GetComponent<GameObjectComponent>().GameObject;
                    b2WorldManager.Instance.DrawPoint(go.transform.position.ToVector2(), 8f, Color.Green);
                }
            }
        }

        public class VirtualCameraLoadSystem : LoadSystem<VirtualCamera>
        {
            protected override void Load(VirtualCamera self)
            {
                foreach (var kv in self.targetIds)
                {
                    CameraTarget target = self.GetChild<CameraTarget>(kv.Value);
                    target.Dispose();
                }
